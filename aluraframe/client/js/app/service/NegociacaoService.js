class NegociacaoService {
  ...
  obterNegociacoesDaSemanaAnterior(funcaoDeCallBack) {
    // https://developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequest
    let xhr = new XMLHttpRequest();
    ...
    xhr.onreadystatechange = () => {
      if (xhr.readyState == 4) {
        if (xhr.status == 200) {
          ...
          console.log(xhr.responseText);
          funcaoDeCallBack('Erro na solicatação AJAX - semana anterior', null);
        }
      }
    };
    xhr.send();
  ...
  obterNegociacoesDaSemanaRetrasada(funcaoDeCallBack) {
    // https://developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequest
    let xhr = new XMLHttpRequest();
    ...
    xhr.onreadystatechange = () => {
      if (xhr.readyState == 4) {
        if (xhr.status == 200) {
          ...
          console.log(xhr.responseText);
          funcaoDeCallBack('Erro na solicatação AJAX - semana retrasada', null);
        }
      }
    };
    xhr.send();
